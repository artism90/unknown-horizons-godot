...
func _on_ready() -> void:
	if not static_body: static_body = $StaticBody3D as StaticBody3D
	if not collision_shape: collision_shape = static_body.get_node("CollisionShape3D") as CollisionShape3D
